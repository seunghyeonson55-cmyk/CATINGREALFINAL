...
def expand_query(q: str) -> dict:
    ...
    prompt = (
        ...
        "■ 형식 규칙: 배우 프로필 분석과 같은 형식으로 써야 검색이 맞는다. "
        "**생김새와 분위기를 하나로 녹여**(생김새가 어떤 분위기를 만드는지) 쓰되, "
        "흔한 일반론('또렷한 눈매')에 그치지 말고 **그 인물 특유의 구체적 인상**을 잡아내라.\n"
        "- desc: 원하는 눈매·콧대·입매·턱선·얼굴형·이목구비 비율 같은 생김새가 어떤 분위기를 만드는지가 "
        "한 호흡으로 드러나게 2~3문장(한국어)으로.\n"
