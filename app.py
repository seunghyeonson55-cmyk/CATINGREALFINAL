...
def analyze_person_faces(face_pngs: list[bytes], detailed: bool = False) -> dict:
    ...
    detail_clause = (
        "■ 얼굴 세밀 모드(최종 결론용): **원초적인 얼굴(보정·메이크업·연출을 걷어낸 맨얼굴 생김새 구조)이 "
        "1순위 기준**이다. 눈매(쌍꺼풀 유무·눈꼬리 각도·눈동자 크기·눈 사이 간격), 눈빛, "
        "미간·눈썹 모양과 짙기, 콧대·콧방울, 입매·입술 두께, 광대·턱선·얼굴형, 이목구비 비율·간격까지 "
        "**아주 세밀하게** 관찰하고 이 구체적 생김새를 **desc의 앞·다수(2/3 이상)로** 적어라. "
        "**가장 중요한 원칙: 분위기는 따로 보는 것이 아니라 '이 얼굴 구조가 만들어내는 결과'로만 적어라. "
        "즉 분위기는 반드시 원초적 얼굴에서 나와야 한다.** "
        "(예: 처진 눈매와 둥근 얼굴형 → 순하고 부드러운 인상 / 날카로운 눈매와 좁은 턱 → 차갑고 시크한 인상) "
...
def expand_query(q: str) -> dict:
    ...
    prompt = (
        ...
        "**원초적 얼굴 생김새 구조(보정·연출 걷어낸 맨얼굴)를 1순위로, desc의 앞·다수(2/3 이상)** 적고, "
        "분위기는 그 생김새에서 나오는 결과로 뒤에 짧게 덧붙여라.\n"
        "- desc: 눈매(쌍꺼풀·눈꼬리 각도·눈동자·눈 사이 간격), 미간·눈썹, 콧대·콧방울, "
        "입매·입술 두께, 광대·턱선·얼굴형, 이목구비 비율·간격 같은 **구체적 생김새를 먼저 자세히** 묘사하고, "
        "그것이 만드는 분위기를 뒤에 이어 2~3문장(한국어)으로.\n"
        "- keywords: 핵심 키워드 6~10개. **앞쪽 다수를 생김새 키워드**(예: 둥근얼굴형·짙은눈썹·오똑한코), "
        "분위기 단어는 뒤에 2~3개만. 이름·작품명 금지.\n"
