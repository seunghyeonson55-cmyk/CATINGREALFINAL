...
def analyze_person_faces(face_pngs: list[bytes], detailed: bool = False) -> dict:
    ...
    detail_clause = (
        "■ 세밀 관찰 모드: 평소보다 한 단계 더 깊이 본다. 눈매(쌍꺼풀·눈꼬리 각도·눈동자·눈 사이 간격), "
        "눈빛·시선의 느낌, 미간·눈썹, 콧대·콧방울, 입매·입술, 광대·턱선·얼굴형, 이목구비 비율의 미묘한 균형, "
        "그리고 그 생김새에서 풍기는 **고유한 분위기(이 사람만의 인상)** 까지 함께 세밀하게 관찰해 "
        "desc에 자연스럽게 녹여라. 흔한 일반론('또렷한 눈매' 수준)에 그치지 말고, "
        "**비슷한 사람과 구분되는 이 사람만의 특징·분위기**를 잡아내라. traits도 0.02 단위로 미세하게 채점.\n"
    ) if detailed else ""
    prompt = (
        ...
        "- desc: 사람이 얼굴을 볼 때처럼, **생김새와 분위기·인상을 하나로 녹여** 3~4문장(한국어)으로. "
        "생김새 특징(눈매·쌍꺼풀·콧대·입매·턱선·얼굴형·이목구비 비율 등)이 어떤 **분위기·인상**"
        "(예: 험상궂은·날카로운·선한·순한·차가운·시크한·청순한·강렬한·도시적인·푸근한 등)을 "
        "만들어내는지가 한 호흡으로 드러나게. **분위기·인상을 충분히 담아라**(분위기 검색이 잘 되게). "
        "단, **이 분위기는 사진의 조명·보정·연출·메이크업이 만든 '사진 무드'가 아니라, "
        "원초적 얼굴(맨얼굴 생김새 구조) 자체에서 나오는 것**이어야 한다. 어두운 조명·진한 화장 때문에 "
        "시크해 보이는 것이 아니라, 눈매·턱선 같은 골격이 본래 그런 인상을 주는지로 판단하라. "
        "예: '짙은 눈썹과 치켜 올라간 눈매, 각진 턱선이 모여 다소 험상궂고 강인한 인상을 준다'. "
        "비슷한 사람과 구분되도록 구체적으로. 실존 연예인 이름·배경지식 사용 금지.\n"
        "- keywords: 이 인물을 구분 짓는 핵심 키워드 5~8개. **생김새 특징과 분위기 단어를 섞어서**"
        "(예: 짙은눈썹, 날카로운눈매, 각진턱, 험상궂은, 강인한). 이름·작품명 금지.\n"
...
def expand_query(q: str) -> dict:
    ...
    prompt = (
        ...
        "■ 형식 규칙: 배우 프로필 분석과 같은 형식으로, **생김새와 분위기·인상을 하나로 녹여** 쓴다. "
        "검색어가 분위기(예: '험상궂은', '선한', '청순한')면 **그 분위기를 충분히 반영**하라.\n"
        "- desc: 원하는 눈매·콧대·입매·턱선·얼굴형·이목구비 비율 같은 생김새가 어떤 분위기·인상을 "
        "만드는지가 한 호흡으로 드러나게 2~3문장(한국어)으로. 분위기 단어를 충분히 담아라.\n"
...
def render_search_controls(prefix: str):
    ...
    with st.container(key=f"{prefix}_filterbox"):
        ...
        c3, c4 = st.columns(2)
        with c3:
            height_min, height_max = st.slider("키 범위 (cm) — 원하는 범위로 직접 설정",
                                               140, 210, (140, 210), key=f"{prefix}_h")
        with c4:
...
FACE_WEIGHT = 0.9   # 사진+텍스트 둘 다일 때: 얼굴 90% + 분위기 10%


def _face_pct(c: float) -> float:
    """SFace 코사인(같은사람 기준선 0.363)을 보기 좋은 0~1 매칭도로 환산.
    다른 사진의 같은 사람은 코사인 0.4~0.6대라, 그대로 쓰면 매칭도가 낮아 보임 → 기준선 보정."""
    c = max(0.0, min(1.0, c))
    if c <= 0.363:
        return c / 0.363 * 0.5             # 0 ~ 기준선 → 0% ~ 50%
    return 0.5 + (c - 0.363) / 0.637 * 0.5  # 기준선 ~ 1.0 → 50% ~ 100%


def _face_image_search(apps, filters, ref_vec, qvec=None, app_emb=None):
    """올린 사진의 얼굴 벡터로 등록 배우와 직접 대조. 텍스트도 있으면 분위기 10% 반영.
    (배우, 매칭도) 내림차순."""
    wv = 1.0 - FACE_WEIGHT
    has_text = qvec is not None and app_emb is not None
    scored = []
    for i, a in enumerate(apps):
        ...
        if av is None:
            continue
        fpct = _face_pct(facevec.cosine(ref_vec, av))      # 얼굴 매칭도(0~1, 기준선 보정)
        if has_text and i < len(app_emb):
            try:
                vcos = max(0.0, float(np.dot(np.asarray(app_emb[i], dtype=np.float32), qvec)))
            except Exception:
                vcos = 0.0
            score = FACE_WEIGHT * fpct + wv * vcos          # 사진+텍스트: 9:1
        else:
            score = fpct                                    # 사진만: 얼굴 100%
        scored.append((a, score))
    scored.sort(key=lambda x: -x[1])
    return [(a, max(0.0, min(1.0, s))) for a, s in scored]
