...
def analyze_person_faces(face_pngs: list[bytes], detailed: bool = False) -> dict:
    ...
    prompt = (
        ...
        "- desc: **추상어('또렷한·세련된·강렬한')는 금지**. 각 부위를 **구체적인 모양·크기·비율**로 "
        "측정하듯 묘사하라(사람마다 달라 구분되도록):\n"
        "  · 눈: 가로 길이(긴/짧은), 세로 높이(큰/작은), 쌍꺼풀(유무·폭), 눈꼬리 각도(올라간/처진/수평), "
        "눈동자 크기, **눈 사이 간격(눈 하나 너비보다 넓/같/좁)**.\n"
        "  · 코: 콧대 높이·길이(높고 긴/낮고 짧은), 콧방울 폭(넓은/좁은), 코끝(둥근/뾰족한/들린).\n"
        "  · 입: 윗입술·아랫입술 두께, 입 너비(눈동자 안쪽선 기준 넓/좁), 입꼬리.\n"
        "  · 얼굴형: 얼굴 길이 대 너비, 이마·광대·턱의 비율, 턱선(갸름/각진/둥근).\n"
        "  · 부위 간 거리·비율: 미간 너비, 인중 길이(긴/짧은), 코끝~윗입술 거리.\n"
        "이렇게 **구체적 생김새를 먼저 충분히** 적고, 끝에 그 생김새가 만드는 분위기를 한 문장만 덧붙여 "
        "3~4문장(한국어). 실존 연예인 이름·배경지식 사용 금지.\n"
        "- keywords: 이 인물을 구분 짓는 **구체적 생김새 키워드** 6~9개"
        "(예: 좁은미간, 긴콧대, 둥근코끝, 짧은인중, 도톰한아랫입술, 올라간눈꼬리). 추상어·이름 금지.\n"
...
def expand_query(q: str) -> dict:
    ...
    prompt = (
        ...
        "■ 형식 규칙(매우 중요): 배우 프로필 분석과 **똑같은 형식**으로 써야 검색이 맞는다. "
        "**추상어('또렷한·세련된') 금지**, 각 부위를 **구체적 모양·크기·비율**로 묘사하라.\n"
        "- desc: 눈(가로길이·세로높이·쌍꺼풀·눈꼬리 각도·눈 사이 간격), 코(콧대 높이·길이·콧방울 폭·코끝), "
        "입(윗/아랫입술 두께·입 너비), 얼굴형(길이 대 너비·턱선), 부위 간 거리·비율(미간·인중·코-입 거리) 같은 "
        "**구체적 생김새를 먼저** 적고, 그것이 만드는 분위기를 끝에 한 문장만 덧붙여 2~3문장(한국어).\n"
        "- keywords: 구체적 생김새 키워드 6~10개(예: 좁은미간, 긴콧대, 둥근코끝, 짧은인중). 추상어·이름 금지.\n"
